...
const sortingGuildId = process.env.SORTING_GUILD; // Add this line to load the guild ID
...
client.on('messageCreate', async message => {
    ...
    if (message.guild.id !== sortingGuildId) {
        return; // Ignore messages from other guilds
    }

    const allowedChannelId = process.env.ALLOWED_CHANNEL_ID;

    // Only check for the wrong channel if the command is '!sortme'
...
client.once('ready', () => {
    ...
    client.user.setPresence({
        activities: [{ name: 'Sorting players!', type: 'PLAYING' }],
        status: 'online',
    });
});
